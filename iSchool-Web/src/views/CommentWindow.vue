...
const refreshCommentObj = async () => {
  // 搜索
  toStatus(1)
  selectedCommentObj.value = { id: -1 }
  isLoading.value = true
  try {
    var res = await searchCommentObj('', '')
    // await sleep(1000) // test
    // var res = { data: commentObj }
  } catch {
    // 请求错误时关闭加载画面
...
onMounted(() => {
  onSearch().then(async () => {
    ...
    if (router.currentRoute.value.query.objId != undefined) {
      ...
      let lit_commentObj = commentObjData.value.filter((item) => {
      ...
      await readMessage({ messageId: router.currentRoute.value.query.messageId }) // 标为已读
    }
  })
})

const showingAddCommentDialog = ref(false) // 是否显示添加点评对象对话框
const formAddCommentObj = ref({
  // 添加点评对象信息表单
  name: '',
  type: ''
})
const onAddCommentObj = async () => {
  // 添加点评对象
...
const inputPlaceholder = computed(() => {
  ...
  return '回复 ' + selectedComment.value.username + '：'
})
...
const onLikeComment2 = async (commentObj) => {
  ...
  await addCommentLikes(commentObj.id)
  // await sleep(1000) // test
  commentObj.liked = true
  commentObj.likes += 1
}
...
const onSelectComment = async (commentObj, commentLevel) => {
  ...
  if (selectedComment.value.id == commentObj.id) {
    ...
    console.log(commentObj, commentLevel)
  ...
  <div style="position: relative; height: 710px">
    ...
    <div ref="window1" class="window1">
      <div style="display: flex">
        ...
        <el-button size="large" type="primary" @click="showingAddCommentDialog = true"
          ...
          <el-scrollbar ref="resultScrollbarRef" wrap-style="transition: all 0.5s;" height="100%">
            ...
            <el-card
              :style="{
                width: '99.5%',
                marginBottom: '10px',
                backgroundColor: item.id == selectedCommentObj.id ? '#eeeeee' : ''
              }"
              v-for="item in commentObjData"
              :key="item"
              shadow="hover"
            >
              <template #header>
                <div style="display: flex; line-height: 30px" @click="onSelectCommentObj(item)">
                  <el-tag size="large" :type="typeColor(item.type)" effect="dark">{{
                    item.type
                  }}</el-tag>
                  <el-text
              ...
              <div style="display: flex">
                <div style="text-align: center" @click="onSelectCommentObj(item)">
                  ...
                  <div style="color: gray">
                    ...
                    >评分/ <span style="font-weight: 700">{{ item.commentCount }}</span
                    >评论
                  </div>
                </div>
                ...
                <div style="text-align: center">
                  <div style="color: gray">
                    立即评分<span style="color: #f2cb51; font-weight: 700; margin-left: 10px">{{
                      item.userScore !== 0 ? item.userScore * 2 : ''
                    }}</span>
                  </div>
                  <el-rate
                    size="large"
                    v-model="item.userScore"
                    @change="commentObj_like(item)"
      ...
      <el-card style="height: 100%" v-show="selectedCommentObj.id != -1">
          ...
          <div style="padding: 10px">
            <div style="display: flex">
              ...
              <div style="text-align: center">
                ...
                <div style="color: gray">
                  <span style="font-weight: 700">{{ selectedCommentObj.count }}</span
                  >评分/<span style="font-weight: 700">{{ selectedCommentObj.commentCount }}</span
                  >评论
                </div>
              </div>
            </div>
        ...
        <div v-loading="isLoading_comment1" style="height: 365px">
          <el-scrollbar ref="comment1ScrollbarRef" height="100%">
            <!-- 一级评论 -->
            <el-empty
              v-show="commentData1.length === 0"
              description="快发出你的第一条评论吧~"
            ...
            <div
              :style="{
                ...
                padding: '10px 2px',
                backgroundColor: item.id === selectedComment.id ? '#eeeeee' : ''
              }"
              v-for="item in commentData1"
              :key="item"
            >
          ...
          <div style="padding: 10px">
            <div style="display: flex">
              ...
              <el-link @click="onLikeComment1(selectedComment1)" :underline="false">
                ...
                <span
                  :style="{ color: selectedComment1.liked ? '#f3c668' : '', marginLeft: '5px' }"
                  >{{ selectedComment1.likes }}</span
                >
              </el-link>
            </div>
