...
const orderContext = computed(() => { // 排序解释文本
    if (orderBy.value === 0)
        return '默认排序'
...
const orderImgUrl = computed(() => { // 排序图标
    if (orderBy.value === 0)
        return '/public/img/defaultOrder.png'
    else if (orderBy.value === 1)
        return '/public/img/timeAsc.png'
    else if (orderBy.value === 2)
...
const onClickChatButton = async () => { // 点击机器人按钮
...
const setRobatButtonStatus = (status) => { // 机器人按钮状态转换函数
    if (status == 0)
...
<template>
    ...
    <div class="chatBox" ref="chatBoxRef">
        ...
        <div style="position: relative; display: flex; height: 5%;">
            ...
            <el-input v-model="chatMessage" size="large" style="width: 80%" :placeholder="chatPlaceholder"
                @keyup.enter="onSendMessage" :disabled="!isChatting || isResulting" />
    ...
    <div style="margin-top: 10px; text-align: center;">
        <div style="display: inline-block;">
            <el-input style="width: 700px; border-radius: 10px;" ref="inputRef" size="large" v-model="searchMessage"
                placeholder="输入您要搜索的内容" class="input-with-select" clearable @keyup.enter="onSearch">
                <template #append>
    ...
    <div v-loading="isLoading">
                ...
                <div style="width: 98%; margin-bottom: 30px; border-bottom: 1px #dcdfe6 solid;"
                    ...
                    <div>
                        <a style="width: 80%;" :href="`/tempView?id=${item.id}`" target="_blank">
                            <el-text class="resultTitle" style="font-size: 25px; max-width: 70%; color: #4c07a2;"
                                truncated><span v-html="item.title"></span></el-text>
                        </a>
                    </div>
        ...
        <div style="text-align: center;">
            <!-- 分页器 -->
            <div style="display: inline-block;">
                <el-pagination ref="pageRef" background layout="prev, pager, next" :page-size="numShowingDataOnOnePage"
                    :total="resultData.counts" @change="onChangePage" :hide-on-single-page="resultData.counts == 0"
                    v-model:current-page="currentPage" />
            </div>
            <span style="margin-left: 2%; color: gray" v-show="resultData.counts != 0">搜索结果：<span>{{ resultData.counts
                    }}</span></span>
        </div>
    </div>
